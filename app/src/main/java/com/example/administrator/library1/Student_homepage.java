...
public class Student_homepage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        book_Id=student_local.getBookList();
        for(int i=0;i<student_local.getBookAmountsetContentView(R.layout.activity_main);();++i) {
            bookList.add(LitePal.find(Book.class, book_Id[i]));                  //得到学生的信息
        }

        RecyclerView recyclerView=(RecyclerView)findViewById(R.id.recycler_view);
        LinearLayoutManager layoutManager=new LinearLayoutManager(this);
        recyclerView.setLayoutManager(layoutManager);
        BookAdapter adapter=new BookAdapter(bookList);                               //这里出现了bug
        recyclerView.setAdapter(adapter);                                         //制作recycler滚动
            ...
            public void onClick(View view) {
                if (book_in_search.getIn_shelf() > 0&&key==true) {
                    if (student_local.getBookAmount() < 10) {
                        int current_book = book_in_search.getIn_shelf();
                        book_in_search.setIn_shelf(current_book - 1);
                        book_in_search.save();
                        book_Id[student_local.getBookAmount()] = book_in_search.id;
                        bookList.add(book_in_search);
